...
void FmcImaging::filtering(float* offLineFmc, float* filter, float* offLineFmc_filted){
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(WaveNum*WaveLength), offLineFmc_filted, filt(filter, offLineFmc_d, WaveLength, taps));
}
