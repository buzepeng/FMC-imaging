...
__global__ void hilbert_transform_kernel(cufftComplex* signals, cufftComplex* filter, cufftComplex *Hilbd int WaveNum, int WaveLength){
    int ix = blockDim.x*blockIdx.x+threadIdx.x;
    int iy = blockDim.y*blockIdx.y+threadIdx.y;
    int idx = iy*WaveLength+ix;

    if(iy<WaveNum && ix<WaveLength){
        int sig_idx = (ix<WaveLength>>1)?(iy*WaveLength+ix):(iy*WaveLength+WaveLength-ix);
        cufftComplex signal_val = signals[sig_idx];
        signals[idx] = signal_val*filter[ix];
    }
}
...
struct thrust_imaging{
    ...
    int WaveNum, WaveLength, col_tof;

    thrust_imaging(float *_offLineFmc_ind, short *_Tof, cufftComplex *_FmcMatHilbert, int _WaveNum, int _WaveLength, int _col_tof):offLineFmc_ind(_offLineFmc_ind), Tof(_Tof), FmcMatHilbert(_FmcMatHilbert), WaveNum(_WaveNum), WaveLength(_WaveLength), col_tof(_col_tof){};
    __device__
    float operator()(int i){
        ...
        for(int s = 0;s<WaveNum;s++){
            int tIndex = int(offLineFmc_ind[2*s]-1), rIndex = int(offLineFmc_ind[2*s+1]-1);
...
class FmcImaging{
    ...
    public:
        short *h_iTof, *d_iTof;
        float *h_offLineFmc, *d_offLineFmc_ind, *d_offLineFmc_data, *d_offLineFmc_filted, *d_TfmImage, *d_filter;
        ...
        void hilbert_transform(float* offLineFmc_filted, cufftComplex* HilbertFilter, cufftComplex* HilbertMat);
...
FmcImaging::FmcImaging(int _WaveNum, int _WaveLength, int _iWaveLength, int _row_tof, int _col_tof, int _NZ, int _NX, int _taps):WaveNum(_WaveNum),WaveLength(_WaveLength),iWaveLength(_iWaveLength),row_tof(_row_tof),col_tof(_col_tof),NZ(_NZ),NX(_NX),taps(_taps)
{
    ...
    cudaMalloc((void**) &d_offLineFmc_ind, 2 * WaveNum *sizeof(float));
    cudaMalloc((void**) &d_offLineFmc_data, WaveLength * WaveNum *sizeof(float));
    cudaMalloc((void**) &d_offLineFmc_filted, WaveLength * WaveNum *sizeof(float));
    cudaMalloc((void**) &d_TfmImage, NZ * NX * sizeof(float));

    cudaMalloc((void**) &d_Hilbert, WaveLength * sizeof(cufftComplex));
    cudaMalloc((void**) &d_HilbertMat, WaveLength * WaveNum * sizeof(cufftComplex));
...
FmcImaging::~FmcImaging(){
    ...
    cudaFree(d_iTof);
    cudaFree(d_offLineFmc_ind);
    cudaFree(d_offLineFmc_data);
    cudaFree(d_offLineFmc_filted);
    cudaFree(d_TfmImage);
    cudaFree(d_filter);
    cudaFree(d_Hilbert);
    cudaFree(d_f_offLineFmc);
    cudaFree(d_HilbertMat);

    cufftDestroy(planForward);
    cufftDestroy(planInverse);
}

void FmcImaging::imaging(float* offLineFmc_ind, short* Tof, cufftComplex* FmcMatHilbert, float* TfmImage, float MindB){
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(NX*NZ), TfmImage, thrust_imaging(offLineFmc_ind, Tof, FmcMatHilbert, WaveNum, WaveLength, col_tof));
...
void FmcImaging::hilbert_transform(float* offLineFmc_filted, cufftComplex* HilbertFilter, cufftComplex *HilbertMat){
    cufftExecR2C(planForward, offLineFmc_filted, this->f_offLineFmc);
    dim3 block(TILEW, TILEH, 1);
    dim3 grid(ceil(WaveLength/block.x)+1, ceil(WaveNum/block.y)+1, 1);
    hilbert_transform_kernel<<<grid, block>>>(this->f_offLineFmc, HilbertFilter, HilbertMat, WaveNum, WaveLength);
    cudaDeviceSynchronize();
    cufftExecC2C(planInverse, f_offLineFmc, f_offLineFmc, CUFFT_INVERSE);
}

void FmcImaging::filtering(float* offLineFmc_data, float* filter, float* offLineFmc_filted){
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(WaveNum*WaveLength), offLineFmc_filted, filt(filter, offLineFmc_data, WaveLength, taps));
}
