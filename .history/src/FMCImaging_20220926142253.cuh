...
void FmcImaging::hilbert_transform(float* offLineFmc_filted, cufftComplex* HilbertFilter, cufftComplex *f_offLineFmc){
    cufftExecR2C(planForward, offLineFmc_filted, this->d_f_offLineFmc);
    dim3 block(TILEW, TILEH, 1);
    dim3 grid(ceil(WaveLength/block.x)+1, ceil(WaveNum/block.y)+1, 1);
    hilbert_transform_kernel<<<grid, block>>>(this->d_f_offLineFmc, HilbertFilter, HilbertMat, WaveNum, WaveLength);
    cudaDeviceSynchronize();
    cufftExecC2C(planInverse, HilbertMat, this->d_f_offLineFmc, CUFFT_INVERSE);
}
