...
__device__ cufftComplex operator / (cufftComplex a, int b){
    cufftComplex res;
    res.x = a.x*b;
    res.y = a.y*b;
    return res;
...
__global__ void hilbert_transform_kernel(cufftComplex* signals, cufftComplex* filter, cufftComplex *HilbertMat, int WaveNum, int WaveLength){
    ...
    if(iy<WaveNum && ix<WaveLength){
        ...
        HilbertMat[idx] = signals[sig_idx]*filter[ix];
    }
}
...
void FmcImaging::hilbert_transform(float* offLineFmc_filted, cufftComplex* HilbertFilter, cufftComplex *f_offLineFmc){
    cufftExecR2C(planForward, offLineFmc_filted, f_offLineFmc);
    dim3 block(TILEW, TILEH, 1);
    dim3 grid(ceil(WaveLength/block.x)+1, ceil(WaveNum/block.y)+1, 1);
    hilbert_transform_kernel<<<grid, block>>>(f_offLineFmc, HilbertFilter, this->d_HilbertMat, WaveNum, WaveLength);
    cudaDeviceSynchronize();
    cufftExecC2C(planInverse, this->d_HilbertMat, f_offLineFmc, CUFFT_INVERSE);
    thrust::transform(thrust::device, f_offLineFmc, f_offLineFmc+WaveLength*WaveNum, f_offLineFmc, [=]__device__(cufftComplex val)->cufftComplex{
