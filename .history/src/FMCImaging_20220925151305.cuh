...
struct zero_phase_filt{
    ...
    zeros_phase_filt(cufftComplex* _signals, cufftComplex* _filter)signals(_signals),filter(_filter){}
    __device_builtin_variable_blockDim
}
...
void FmcImaging::freq_domain_filtering(float* offLineFmc, cufftComplex* f_offLineFmc, cufftComplex* H){
    ...
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator(WaveNum*WaveLength), temp, f_offLineFmc, [=]__device__(int val, cufftComplex c)->cufftComplex{
        return c*powf(cuCabsf(c));
    })
    // dim3 block(TILEW, TILEH, 1);
    // dim3 grid(ceil(WaveLength/block.x)+1, ceil(WaveNum/block.y)+1, 1);
