...
class FmcImaging{
    ...
    public:
        short *h_iTof, *d_iTof;
        float *h_offLineFmc, *d_offLineFmc, *d_offLineFmc_filted, *d_TfmImage, *d_f_filter;
...
void FmcImaging::filtering(float* offLineFmc, float* f_filter, cufftComplex* f_offLineFmc){
...
void FmcImaging::get_filter(float f1, float f2, float* f_filter){
    //generate bandpass filter
    float* d_filter;
    cuffeHandle planFilter;
    cufftPlan1d(&plan_fft, WaveLength, CUFFT_R2C, 1);
    cudaMalloc((void **)&d_filter, WaveLength, WaveLength*sizeof(cufftComplex));
