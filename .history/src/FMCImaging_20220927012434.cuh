...
struct thrust_imaging{
    ...
    float operator()(int i){
        ...
        for(int s = 0;s<WaveNum;s++){
            int tIndex = int(offLineFmc_ind[iWaveLength*s]-1), rIndex = int(offLineFmc_ind[2*s+1]-1);
...
void FmcImaging::imaging(float* offLineFmc, short* Tof, cufftComplex* FmcMatHilbert, float* TfmImage, float MindB){
    thrust::transform(thrust::device, thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(NX*NZ), TfmImage, thrust_imaging(offLineFmc, Tof, FmcMatHilbert, WaveNum, WaveLength, col_tof));
