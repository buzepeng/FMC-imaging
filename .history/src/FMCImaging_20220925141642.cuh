...
struct thrust_imaging{
    ...
    float operator()(int i){
        ...
        for(int s = 0;s<WaveNum;s++){
            ...
            if(tIndex>=r)
            if(tIndex>=64)  printf("illegal tIndex\n");
            if(rIndex>=64)  printf("illegal rIndex\n");
            int trTofIndex = int(Tof[tIndex*col_tof+i]+Tof[rIndex*col_tof+i]), fmc_ind = s*WaveLength+trTofIndex-1;
            if(fmc_ind>=WaveLength*WaveNum) printf("illegal address\n");
...
void FmcImaging::freq_domain_filtering(float* offLineFmc, cufftComplex* f_offLineFmc, cufftComplex* H){
    float *temp;
    cudaMalloc((void **)&temp, WaveNum*WaveLength*sizeof(float));
    for(int i = 0;i<WaveNum;i++){
        cudaMemcpy(temp+i*WaveLength, offLineFmc+i*iWaveLength+2, WaveLength*sizeof(float), cudaMemcpyDeviceToDevice);
        cudaDeviceSynchronize();
    }
