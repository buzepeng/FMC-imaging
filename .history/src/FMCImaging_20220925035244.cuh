...
struct thrust_imaging{
    float *offLineFmc, *FmcMatHilbert;
    short *Tof;
    int WaveNum, iWaveLength, WaveLength, col_tof;

    thrust_imaging(float *_offLineFmc, short *_Tof, float *_FmcMatHilbert, int _WaveNum, int _iWaveLength, int _WaveLength, int _col_tof):offLineFmc(_offLineFmc), Tof(_Tof), FmcMatHilbert(_FmcMatHilbert), WaveNum(_WaveNum), iWaveLength(_iWaveLength), WaveLength(_WaveLength), col_tof(_col_tof){};
    __device__
...
void FmcImaging::imaging(float* offLineFmc, short* Tof, float* FmcMatHilbert, float* TfmImage, float MindB){
...
void FmcImaging::hilbert_transform(cufftComplex* f_offLineFmc, cufftComplex* HilbertFilter, float* FmcMatHilbert){
    dim3 block(TILEW, TILEH, 1);
    ...
    cudaDeviceSynchronize();
    cufftExecC2R(planInverse, f_offLineFmc, FmcMatHilbert);
    thrust::transform(thrust::device, FmcMatHilbert, FmcMatHilbert+WaveNum*WaveLength, [=]__device__(float val)->float{
