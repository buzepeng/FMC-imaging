...
struct thrust_imaging{
    ...
    float operator()(int i){
        ...
        for(int s = 0;s<WaveNum;s++){
            ...
            int trTofIndex = int(Tof[tIndex*col_tof+i]+Tof[rIndex*col_tof+i]);
            if(s*WaveLength+trTofIndex-1>=WaveNum*WaveLength)   printf("illegal address\n");
            // float val1 = FmcMatHilbert[s*WaveLength+trTofIndex-1].x, val2 = FmcMatHilbert[s*WaveLength+trTofIndex-1].y;
            if(i==986403){
                printf("%d, %d, %d, %f, %f\t", s, trTofIndex, val1, val2);
            }

            // real += val1;
            // imag += val2;
            complex_sum = complex_sum + FmcMatHilbert[s*WaveLength+trTofIndex-1];
        }
        // printf("%d, %f, %f\n", i, real, imag);
        // return sqrtf(powf(real, 2)+powf(imag, 2));
        if(i==986403){
            printf("%d, %d, %d, %f, %f\t", cuCabsf(complex_sum));
        }
