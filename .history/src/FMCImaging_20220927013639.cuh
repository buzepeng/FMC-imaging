...
struct thrust_imaging{
    ...
    thrust_imaging(float *_offLineFmc_ind, short *_Tof, cufftComplex *_FmcMatHilbert, int _WaveNum, int _WaveLength,int _iWaveLength, int _col_tof):offLineFmc_ind(_offLineFmc_ind), Tof(_Tof), FmcMatHilbert(_FmcMatHilbert), WaveNum(_WaveNum), WaveLength(_WaveLength), iWaveLength(_iWaveLength), col_tof(_col_tof){};
    __device__
    float operator()(int i){
        ...
        for(int s = 0;s<WaveNum;s++){
            int tIndex = int(offLineFmc[iWaveLength*s]-1), rIndex = int(offLineFmc_ind[iWaveLength*s+1]-1);
...
struct filt{
    ...
    float operator()(int idx){
        float val = 0;
        int i = idx%WaveLength-2, upbound = i<b_size?i+1:b_size;
        if(i<0) return x[idx];
...
void FmcImaging::hilbert_transform(float* offLineFmc_filted, cufftComplex* HilbertFilter, cufftComplex *f_offLineFmc){
    cufftExecR2C(planForward, &offLineFmc_filted[2], f_offLineFmc);
    dim3 block(TILEW, TILEH, 1);
